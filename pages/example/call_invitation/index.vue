<template>
	<view class="container">
		...
		<view class="user-box">
			...
			<!-- <image class="avatar" :src="`https://api.multiavatar.com/${userID}.svg?apikey=XqHm465NYsdLfb`"></image> -->
		...
		<view class="content">
			<view class="title-box">
				...
				<!-- <view class="desc">输入受邀者的用户 ID，用“,”分隔</view> -->
			</view>
		...
		<view class="bottom-box">
			<p>点击 “通话”，即表示您同意</p>
			<p>我们的 <span class="link" @click="goto('/pages/example/terms')">服务条款</span> 和 <span class="link" @click="goto('/pages/example/privacy')">隐私政策</span></p>
		</view>
...
// 引入 prebuiltCall
import { ZegoUIKitPrebuiltCall, ZegoCallScenario, ZegoUIKitUser, makePrebuiltListenerID } from '../../../zego_uikit_prebuilt_call/src/index';
...
import ZegoSendCallInvitationButton from '../../../zego_uikit_prebuilt_call/src/components/ZegoSendCallInvitationButton.vue';
import InvitationCall from '../../../zego_uikit_prebuilt_call/src/pages/call_invitation/InvitationCall.vue';
...
onMounted(() => {
	userName.value = `user_${userID.value}`;
	...
	const config = {
		...
		mode: ZegoCallScenario.CALL_INVITATION,
		globalPagePath: "/zego_uikit_prebuilt_call/src/pages/call_invitation/InvitationCall",
...
const sendCallback = (code: number, message: string, errorInvitees: string[]) => {
	...
	switch (code) {
		case 0:
			// 成功
			showUIKit.value = true;
			break;
		case 100001:
			showToast('用户 ID 为空！');
			break;
		case 6000001:
			showToast('用户 ID 填写有误');
			break;
		case 6000281:
			showToast('输入的用户不在线或不存在');
			break;
		case 100003:
			showToast(message); // 暂不支持多人
			break;
		case 6000104:
			showToast('网络异常，请检查您的网络');
			break;
		default:
			showToast('发送呼叫邀请失败，请稍后再试');
	}
}
